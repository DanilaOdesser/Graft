      ...
      {ordered.map((n, i) => (
        <div key={n.id || i} className="group rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-3 hover:border-[var(--color-border-bright)] transition-colors">
          ...
          <div className="flex gap-1 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {n.id && n.source === "ancestor" && (
              <button
                onClick={() => handleExport(n)}
                disabled={exportingId === n.id}
                className="text-[10px] text-[var(--color-text-faint)] hover:text-[var(--color-blue,theme(colors.blue.600))] px-2 py-0.5 rounded hover:bg-blue-50 transition-colors disabled:opacity-50"
              >
