...
- [ ] Decide on Python version, Node version, formatter/linter (ruff + prettier).
- [ ] Create Postgres database on Supabase. Note the connection string.
- [ ] Repo scaffolding: `/db`, `/backend`, `/frontend`, `/scripts`, `/docs`.
- [ ] `db/migrations/` with a simple migration runner (Alembic, or hand-rolled `psql -f`).
- [ ] Shared `.env.example` and `Makefile` targets: `make db-reset`, `make seed`, `make dev`.
- [ ] Branch protection: PRs only, no direct commits to `main`.
...
### Dev A
- [ ] DDL for `users`, `conversations`, `branches` (3 tables). Foreign keys, defaults, `updated_at` trigger on `conversations`.
- [ ] DDL for `nodes` including the `content_tsv` generated column:
  ```sql
  content_tsv tsvector GENERATED ALWAYS AS (to_tsvector('english', content)) STORED
  ```
- [ ] **Closure-table trigger**: `AFTER INSERT ON nodes` populates `node_ancestry` by copying the parent's ancestor rows + adding self. `AFTER UPDATE ON nodes` (parent change → re-derive — defer if mutability isn't needed for v1).
- [ ] Indexes owned by Dev A:
...
### Dev B
- [ ] DDL for `node_ancestry`, `node_summaries`, `context_pins`, `context_imports`, `tags`, `node_tags`, `branch_shares` (7 tables).
- [ ] **Trigger tests**: write SQL that inserts a small DAG and asserts `node_ancestry` contents are correct (depth values + ancestor-of-self row + transitive ancestors). Run as part of `make db-test`.
- [ ] Indexes owned by Dev B:
...
- [ ] Maintain `schema.dbml` and export the dbdiagram.io PNG into `docs/`.
...
### Dev A
- [ ] `scripts/seed_users.py` — ~20 users, deterministic UUIDs (so Dev B's seeds can reference them).
- [ ] `scripts/seed_conversations.py` — ~50 conversations across users, ~3 branches per conversation (incl. `main`).
- [ ] Backfill `default_branch_id` and `root_node_id` after Dev B's nodes seed runs.

### Dev B
- [ ] `scripts/seed_nodes.py` — ingest 5–10 real-ish chat transcripts (split into messages; chain `parent_id`). Trigger should auto-fill `node_ancestry`.
- [ ] `scripts/seed_pins.py`, `seed_imports.py`, `seed_summaries.py` — sprinkle pins, cross-branch imports (so Q1's union has work to do), and a few summary nodes.
- [ ] `scripts/seed_social.py` — tags, node_tags, branch_shares via Faker.
- [ ] `make seed` runs everything in order.
...
### Dev A
- [ ] **Query 1 — Context assembly**: implement parameterized SQL from `queries.sql`. Wrap in a SQL function `assemble_context(node_id uuid, budget int)` returning the ordered set.
- [ ] Benchmark: 1k nodes, 10k nodes, 100k nodes; chart cumulative time.
- [ ] Verify the closure-table index is used (no Seq Scan on `node_ancestry`).

### Dev B
- [ ] **Query 2 — Branch divergence**: parameterized SQL; wrap as `branch_diverge(a uuid, b uuid)`.
- [ ] **Query 3 — FTS search**: parameterized SQL using `websearch_to_tsquery`; wrap as `search_nodes(user_id uuid, q text, k int)`.
- [ ] Benchmark Q3 with and without the GIN index (drop/recreate to show speedup).
- [ ] Confirm Q2 uses `idx_ancestry_desc_depth`.
...
### Dev A
- [ ] `backend/models/` — SQLAlchemy models for `users`, `conversations`, `branches`, `nodes`.
- [ ] Endpoints:
...
- [ ] **LLM stub agent**: `POST /agent/turn` takes `{node_id, user_message}`, calls `assemble_context`, sends to Claude API, persists assistant reply as a new node, returns it.

### Dev B
- [ ] `backend/models/` — SQLAlchemy models for `node_ancestry`, `node_summaries`, `context_pins`, `context_imports`, `tags`, `node_tags`, `branch_shares`.
- [ ] Endpoints:
...
- [ ] **CLI ingestion tool**: `python -m scripts.ingest <transcript.txt>` creates conversation + main branch + chained nodes.
...
### Dev A
- [ ] App scaffold (Vite + React + Router).
- [ ] **Branch DAG view**: render the node graph for a conversation (use `react-flow` or hand-drawn SVG). Color-code by branch.
- [ ] **Conversation reader**: select a node → show assembled context + the message thread up to it.
- [ ] Hook up `POST /agent/turn` to a "send message" box.

### Dev B
- [ ] **Cherry-pick UI**: from a node in any branch, "import into…" dropdown of target branches.
- [ ] **Pins UI**: pin/unpin node on current branch; show pinned panel.
- [ ] **Search UI**: search bar → list of `GET /search` results with branch + conversation crumbs; click → navigate to node.
...
### Dev A
- [ ] Push Postgres schema + seed to Supabase.
- [ ] Deploy backend to Render (or PythonAnywhere). Set env vars. Confirm `/health` is up.
- [ ] Record demo video (golden path + branch divergence).

### Dev B
- [ ] Deploy frontend to Vercel. Wire `VITE_API_URL` to Dev A's backend.
- [ ] Write `docs/writeup.md`: domain description, entities, three critical user paths, schema-image embed, query explanations, index justifications, benchmark numbers.
- [ ] Add the deployed URL + demo video link to `README.md`.
